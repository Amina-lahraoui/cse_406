...
export default function Auth() {
    ...
    const validate = () => {
        ...
        if (!auth.password) {
            ...
            } else if (!/(?=.*[a-z])/.test(auth.password) || !/(?=.*[A-Z])/.test(auth.password) || !/(?=.*\d)/.test(auth.password) || !/(?=.*[!@#$%^&*(),.?":{}|<>])/.test(auth.password)) {
            ...
            <header className="relative z-20 pt-4 px-6">
                <div className="max-w-7xl mx-auto flex justify-between items-center">
                    <div className="flex items-center space-x-4">
                        <div className={`flex items-center space-x-2 transition-colors duration-1000 ${isDarkTheme ? 'text-white/80' : 'text-gray-700'}`}>
                            <MapPin className="w-4 h-4" />
                            <span className="text-sm">{t("header.location")}</span>
                        </div>
                    </div>
                    <div className={`flex items-center px-3 py-2 rounded-full p-1 cursor-pointer transition-all duration-300 ${isDarkTheme ? 'text-white/80 hover:bg-white/5' : 'text-gray-700 hover:bg-black/5'
                        }`}>
